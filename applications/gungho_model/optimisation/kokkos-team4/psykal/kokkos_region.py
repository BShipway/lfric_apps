...
'''
Shared body of the 'kokkos-team4' scripts that capture a Kokkos region.

This is the 'kokkos-team4' copy of the 'kokkos' profile, and it differs from
it in this file alone -- so that

    diff -r optimisation/kokkos/psykal optimisation/kokkos-team4/psykal

states the whole of the difference rather than a reader having to look for it.
The difference is TEAM_SIZE below, passed to LFRicKokkosTrans as its
'team_size' option, which forces every hierarchical region's team to four
members.

It exists because Kokkos::AUTO, which the default profile leaves in the
policy, is **one member** on the OpenMP backend. A default host build
therefore runs every TeamVectorRange loop, every Kokkos::single wrapper and
every team barrier on a team of one, where each is a no-op or a plain loop:
the shape is generated and compiled but never exercised. Forcing four members
is the only way a host build makes that machinery do work, and so the only way
the checksums say anything about whether it is correct.

The build must run with **at least four OpenMP threads**, or Kokkos refuses
the policy at launch rather than silently shrinking the team.

Its time is **not a measurement**. One team of four running the whole league
serialises the cells behind a rendezvous at every barrier, which is a slower
arrangement than the default profile's by construction. This is a correctness
build; the performance figures come from the 'kokkos' and 'kokkos-omp'
profiles.

Each of the scripts under this root names one invoke and one coded kernel
within it. The
...
#: The number of members every hierarchical region's team is forced to. Four
#: rather than more because the reference run has few enough cells that a
#: larger team would leave members idle on the shorter columns, and more than
#: one because one is what Kokkos::AUTO already gives on a host.
TEAM_SIZE = 4
...
def capture(psyir, script, invoke, kernel):
    ...
    source = LFRicKokkosTrans().apply(
        target_loop(psyir, invoke, kernel),

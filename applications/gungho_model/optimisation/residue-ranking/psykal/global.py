...
##############################################################################
'''
The Fortran model with every cell loop calipered: 'region:captured' for a loop the whole-model Kokkos capture takes, and 'region:residue:<kernel>' for one it leaves, one name per residue kernel so the residue is RANKED. The sibling 'residue-census' gives the two-row split; this tree says which residue kernels the time is in. Its manifest is a symlink to the sibling's, so the two cannot disagree.
...
def trans(psyir):
    ...
    captured, residue = census_by_capture(psyir, read_capture_manifest(MANIFEST), residue_per_kernel=True)
    print(f"residue-census: {captured} loop(s) under region:captured, "
          f"{residue} under region:residue:<kernel>")
